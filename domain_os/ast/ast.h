...
#define SEGMAP_FOR_SEG(seg)     ((segmap_entry_t*)((char*)SEGMAP_BASE + ((seg) << 7)))
...
extern void NETWORK_$AST_GET_INFO(void *uid_info, uint16_t *flags, void *attrs, status_$t *status);
...
void AST_$TOUCH_AREA(aste_t *aste, uint32_t mode, uint16_t start, uint16_t count,
                     status_$t *status, uint16_t flags);
