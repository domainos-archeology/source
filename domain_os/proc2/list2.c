 ...
 * processes with flags 0x180 set (valid and bit 7 of low byte).
    ...
    *last_index = 0;
    ...
    if (status == 0x00120035) {
        ...
        for (slot = 1; slot <= 57; slot++) {
            ...
            /* Check if process is valid and has bit 7 set:
             * flags & 0x100 (valid) and flags & 0x80 (high bit of low byte)
             */
