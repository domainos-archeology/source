*
* SVC_$TRAP6 - Domain/OS TRAP #6 System Call Dispatcher
*
* This is the entry point for 6-argument system calls in Domain/OS.
* User programs invoke system calls via TRAP #6 with:
...
*   0x00e7b1d8: SVC_$TRAP6 main entry
*
* Address space check:
...
*   2. Look up handler in SVC_$TRAP6_TABLE (defined in svc_tables.c)
...
SVC_$TRAP6:
        cmp.w   #$3B,D0                 * Check syscall number limit
        bcc.b   SVC_$TRAP6_INVALID_JUMP * Branch if D0 >= 59

        lsl.w   #2,D0                   * D0 = syscall_num * 4 (table index)
        lea     SVC_$TRAP6_TABLE,A0     * A0 = table base (from svc_tables.c)
        ...
        bhi.b   SVC_$TRAP6_ILLEGAL_USP_JUMP * Branch if USP in kernel space

        * Validate and push arg6 (offset 0x18)
        move.l  (0x18,A1),D0            * D0 = arg6
        cmp.l   D0,D1                   * Check arg6 < 0xCC0000
        bls.b   SVC_$TRAP6_BAD_PTR_JUMP * Branch if arg6 >= boundary
        ...
        * Clean up and return to user mode
        adda.w  #$18,SP                 * Remove 6 arguments (24 bytes)
...
*----------------------------------------------------------------------
        ...
        .extern SVC_$TRAP6_TABLE        * Syscall table (defined in svc_tables.c)
