...
|   0x00e7b240: SVC_$TRAP8 main entry
|
...
SVC_$TRAP8:
        lea     SVC_$TRAP8_ARGCOUNT,%a1 | A1 = argcount table (0xe7be4a)
        cmp.w   #0x38,%d0               | Check syscall number limit
        bcc.b   SVC_$TRAP8_INVALID      | Branch if D0 >= 56

        link    %a6,#0                  | Create stack frame
        clr.w   %d1                     | Clear high byte of D1
        move.b  (0,%a1,%d0:w),%d1       | D1 = argcount for this syscall
        bmi.b   SVC_$TRAP8_NO_ARGS      | If negative, skip arg validation
        ...
        bhi.b   SVC_$TRAP8_ILLEGAL_USP  | Branch if in kernel space
...
SVC_$TRAP8_ARG_LOOP:
        ...
        dbls    %d0,SVC_$TRAP8_ARG_LOOP | Loop while valid and count > 0
        bls.b   SVC_$TRAP8_BAD_PTR_JUMP | If last arg invalid, error
...
SVC_$TRAP8_NO_ARGS:
        ...
        lea     SVC_$TRAP8_TABLE,%a1    | A1 = handler table (0xe7bd6a)
...
SVC_$TRAP8_INVALID:
        ...
        lea     SVC_$TRAP8_TABLE,%a0    | A0 = handler table base
        cmpa.l  %a0,%a1                 | Compare with current position
        bne.b   SVC_$TRAP8_INVALID_2    | If not at start, different error
        unlk    %a6                     | Clean up frame
SVC_$TRAP8_INVALID_2:
        move.l  #0x00120007,%d0         | status_$fault_invalid_SVC_code
        bra.b   SVC_$TRAP8_ERROR

SVC_$TRAP8_ILLEGAL_USP:
        unlk    %a6                     | Clean up frame
        jmp     FIM_$ILLEGAL_USP        | Jump to illegal USP handler

|----------------------------------------------------------------------
| SVC_$TRAP8_ERROR - Common error handling path
|
...
|----------------------------------------------------------------------
        ...
        .extern SVC_$TRAP8_TABLE        | Handler table (defined in svc_tables.c)
        .extern SVC_$TRAP8_ARGCOUNT     | Argument count table (defined in svc_tables.c)
