...
typedef struct dtte {
    ...
    m68k_ptr_t      handler_ptr;            // 0x24: handler pointer (copied to TTY struct offset 4)
    ...
    int16_t         discipline;             // 0x34: terminal discipline (0=TTY, 1=disable alt, 2=enable alt, 3=SUMA)
...
// Verify structure size
_Static_assert(sizeof(term_line_data_t) == 0x4dc, "term_line_data_t must be 1244 bytes");
