 ...
 *   - Ports: Network interface bindings for a channel (up to 8 ports per channel)
...
typedef struct xns_$channel_t {
    ...
    uint8_t     port_active[8];         /* 0xDC-0xE3: Per-port active flags (bit 7 = active) */
...
typedef struct xns_$idp_state_t {
    ...
    xns_$channel_t channels[XNS_MAX_CHANNELS];  /* 0x314-0x7B3: actually relative to full state base */
...
#define XNS_OPEN_FLAG_NO_ALLOC      0x08    /* Don't allocate socket (OS internal use) */
...
#define status_$xns_incompatible_flags      0x3B0018  /* Incompatible flags (bind+noalloc) */
#define status_$xns_incompatible_flags2     0x3B0019  /* Incompatible flags (connect+noalloc) */
#define status_$xns_broadcast_no_addr       0x3B001A  /* Broadcast requires address */
#define status_$xns_local_addr_in_use       0x3B001B  /* Local address already in use */
#define status_$xns_connect_bind_conflict   0x3B001C  /* Connect and bind conflict */
 ...
 */
void XNS_IDP_$RECEIVE(uint16_t *channel, void *recv_params, status_$t *status_ret);
 ...
 */
void XNS_IDP_$OS_SEND(int16_t *channel, void *send_params, uint16_t *checksum_ret,
 ...
 */
void XNS_IDP_$OS_DEMUX(void *packet_info, int16_t *port, void *param3, status_$t *status_ret);
 ...
 */
void XNS_IDP_$OS_ADD_PORT(uint16_t *channel, uint16_t *port, status_$t *status_ret);
 ...
 */
void XNS_IDP_$OS_DELETE_PORT(uint16_t *channel, uint16_t *port, status_$t *status_ret);
