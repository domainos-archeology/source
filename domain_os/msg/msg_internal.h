...
#define MSG_OFF_OWNERSHIP       0x1D8   /* Socket ownership bitmaps (8 bytes per socket) */
...
{
#if defined(M68K)
    uint8_t *bitmap = (uint8_t *)(MSG_$DATA_BASE + MSG_OFF_OWNERSHIP + socket * 8);
 ...
 */
status_$t MSG_$$RCV(msg_$socket_t *socket, void *buffer, uint32_t *length,
                    msg_$desc_t *desc);

/*
 * Network service callback for incoming messages
 */
void MSG_$NET_SERVICE(void *param);
